...
import {
  ArrowLeft, Calendar, Users, FileText, ExternalLink,
...
function SimpleCAMap({ passRate }: { passRate: number }) {
  // SVG-based schematic California county map split by region
  const regions = CA_REGIONS.map(r => ({
    ...r,
    color: REGION_COLORS[r.name],
    // Simulate county-level variation around the overall pass rate
    yesShare: Math.min(100, Math.max(0, passRate * 100 + (Math.random() * 20 - 10))),
  }));
  ...
  return (
    <div className="grid grid-cols-2 gap-4">
      {CA_REGIONS.map(region => {
        ...
        // Use a seeded pseudo-variation per region
        const seed = region.name.charCodeAt(0) % 20 - 10;
        const yes = Math.min(99, Math.max(1, Math.round(passRate * 100 + seed)));
        const no = 100 - yes;
        return (
          <div key={region.name} className="border border-slate-200 bg-white p-4">
            <div className="flex items-center gap-2 mb-3">
              <div className="w-3 h-3 rounded-sm" style={{ background: col }} />
              <span className="text-xs font-semibold uppercase tracking-widest text-slate-600 font-serif">
                {region.name}
              </span>
            </div>
            <div className="flex justify-between text-sm mb-1 font-serif">
              <span className="text-indigo-700 font-bold">Yes {yes}%</span>
              <span className="text-slate-400 font-bold">No {no}%</span>
            </div>
            <div className="h-2 bg-slate-100 overflow-hidden rounded-none">
              <div className="h-full" style={{ width: `${yes}%`, background: col }} />
            </div>
      ...
      <p className="col-span-2 text-xs text-slate-400 font-serif italic">
        * Regional breakdowns are modeled estimates based on historical voting patterns by county. Interactive county-level map in development.
      </p>
    </div>
  );
}
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        // Fetch similar propositions by category
        try {
      ...
      <section className="py-5 bg-white border-b border-slate-200">
        <div className="container mx-auto px-4">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {[
              { icon: Calendar,   label: 'Election Date', value: formatDate(proposition.electionDate) },
              ...
              { icon: DollarSign, label: 'Support Raised',    value: proposition.finance ? formatCurrency(proposition.finance.totalSupport) : 'N/A' },
              { icon: DollarSign, label: 'Opposition Raised', value: proposition.finance ? formatCurrency(proposition.finance.totalOpposition) : 'N/A' },
          ...
          <Tabs defaultValue="details" className="space-y-6">
            ...
            <TabsContent value="visuals">
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                {/* Vote split / passage probability */}
                {voteData && (
                  <Card className="border border-slate-200">
                    <CardHeader className="border-b border-slate-200">
                      <CardTitle className="text-lg font-bold text-slate-900"
                  ...
                  <Card className="border border-slate-200 lg:col-span-2">
                    ...
                    <CardContent className="pt-6">
                      ...
                      <p className="text-xs text-slate-400 font-serif mt-2">
                        <span className="inline-block w-3 h-3 bg-indigo-600 mr-1 align-middle" /> Passed &nbsp;
                        <span className="inline-block w-3 h-3 bg-slate-400 mr-1 align-middle" /> Failed
                      </p>
                    </CardContent>
                  </Card>
                )}

                {/* Category pass rate from R analysis data */}
                <Card className="border border-slate-200 lg:col-span-2">
                  <CardHeader className="border-b border-slate-200">
                    <CardTitle className="text-lg font-bold text-slate-900"
                      style={{ fontFamily: "'Playfair Display', Georgia, serif" }}>
                  ...
                  <CardContent className="pt-6">
                    ...
                    <p className="text-xs text-slate-400 font-serif mt-2">
                      Based on California ballot proposition data 1980–2025. Source: Proposition_Data.csv analysis.
                    </p>
                  </CardContent>
                </Card>
              ...
              <Card className="border border-slate-200">
                <CardHeader className="border-b border-slate-200">
                  <div className="flex items-start justify-between">
                    ...
                    <span className="text-xs bg-amber-100 text-amber-800 border border-amber-300 px-2 py-1 font-serif uppercase tracking-widest">
                      Interactive Map In Development
                    </span>
                  </div>
                </CardHeader>
                <CardContent className="pt-6">
                  <div className="bg-slate-50 border border-slate-200 rounded p-4 mb-6 font-serif text-sm text-slate-600">
                    <strong>Note:</strong> A fully interactive county-level map is currently in development using real precinct-level data. The regional breakdown below uses modeled estimates based on historical voting patterns for similar {proposition.category.replace(/_/g, ' ')} propositions.
                  </div>

                  {/* Schematic map placeholder + regional bars */}
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    {/* CA outline schematic */}
                    <div className="flex flex-col items-center justify-center">
                      <svg viewBox="0 0 200 280" className="w-48 h-64 drop-shadow-sm" aria-label="California regions map">
                        {/* Simplified CA shape by region */}
                        {/* Northern */}
                        <path d="M60,10 L140,10 L145,50 L130,55 L120,70 L60,70 Z"
                          fill={REGION_COLORS['Northern CA']} opacity="0.85" />
                        {/* Bay Area */}
                        <path d="M55,70 L120,70 L115,120 L95,130 L70,125 L55,110 Z"
                          fill={REGION_COLORS['Bay Area']} opacity="0.85" />
                        {/* Central Valley */}
                        <path d="M120,70 L150,75 L155,160 L115,165 L115,120 Z"
                          fill={REGION_COLORS['Central Valley']} opacity="0.85" />
                        {/* Southern */}
                        <path d="M55,110 L70,125 L95,130 L115,165 L155,160 L160,220 L80,240 L55,200 Z"
                          fill={REGION_COLORS['Southern CA']} opacity="0.85" />
                        {/* Labels */}
                        {[
                          ...
                          <text key={label} x={x} y={y} textAnchor="middle"
                            fontSize="9" fill="white" fontWeight="bold" fontFamily="Georgia">
                            {label}
                          </text>
                        ))}
                      </svg>
                    ...
                    <div>
                      ...
                      <p className="text-xs text-slate-400 font-serif mt-4 italic">
                        ...
                        Real precinct data will replace these estimates when available.
                      </p>
                    </div>
                  </div>
                </CardContent>
            ...
            <TabsContent value="similar">
              <div className="space-y-6">
                  ...
                  <Card className="border border-slate-200">
                    <CardContent className="py-16 text-center">
                      ...
                      <p className="text-slate-500 font-serif">
                        No similar propositions found in this category for the current data set.
                      </p>
                ...
                {/* Category pass rate context card */}
                  ...
                  <CardContent className="py-5 flex items-start gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-indigo-700 font-serif leading-relaxed">
                        ...
                        {proposition.category === 'criminal_justice' && 'Criminal justice propositions pass about 53% of the time, though results vary significantly based on prevailing public safety concerns at the time of the election.'}
                        ...
                        {proposition.category === 'government' && 'Government reform measures have mixed outcomes, with pass rates around 50% depending on whether changes are seen as limiting or expanding bureaucracy.'}
